...
import { Plus, FileText, ExternalLink, Copy, Check, BarChart2, Users, LogOut } from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-50 flex">
      <aside className="w-64 bg-[#0F1F3D] text-white flex-col shrink-0 hidden md:flex">
        ...
        <nav className="flex-1 px-4 space-y-1 mt-4">
          ...
          <Link href="/recruiter/candidates" className="flex items-center gap-3 px-4 py-3 rounded-lg text-slate-400 hover:bg-white/5 hover:text-white transition-colors">
            <Users className="w-5 h-5" /> Candidates
          </Link>
          <Link href="/recruiter/interviews" className="flex items-center gap-3 px-4 py-3 rounded-lg bg-white/10 text-white font-medium">
            <FileText className="w-5 h-5 text-[#67E8F9]" /> Interviews
          </Link>
        </nav>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <header className="bg-white border-b border-slate-200 px-6 py-4 sticky top-0 z-10">
          <div className="max-w-5xl mx-auto flex items-center justify-between">
            ...
            <Link href="/recruiter/create-interview" className="flex items-center gap-2 bg-[#2563EB] text-white px-4 py-2.5 rounded-lg font-semibold hover:bg-blue-700 transition-colors text-sm">
              <Plus className="w-4 h-4" /> New Interview
            </Link>
          </div>
        </header>

        <main className="flex-1 max-w-5xl mx-auto w-full mt-8 px-6 pb-20">
          {loading ? (
